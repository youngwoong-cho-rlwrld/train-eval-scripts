#!/bin/bash
#SBATCH --job-name=baseline_scratch_eval
#SBATCH --partition=l40s-gpu
#SBATCH --nodes=1
#SBATCH --gpus-per-node=2
#SBATCH --time=08:00:00
#SBATCH --output=/fsx/rlwrld/youngwoong_cho/logs/baseline_scratch_eval_%j.out
#SBATCH --error=/fsx/rlwrld/youngwoong_cho/logs/baseline_scratch_eval_%j.err
#SBATCH --comment="baseline_scratch: eval-only on existing final checkpoint"
...
if [ -n "$SLURM_SUBMIT_DIR" ]; then
    ...
    elif [ -d "$SLURM_SUBMIT_DIR/skt/$PARENT_DIR" ]; then
        EXP_DIR="$SLURM_SUBMIT_DIR/skt/$PARENT_DIR"
    else
        EXP_DIR="$SLURM_SUBMIT_DIR"
    fi
else
...
GROOT_DIR="/fsx/rlwrld/youngwoong_cho/workspace/gr00t"
ISAAC_DIR="/fsx/rlwrld/youngwoong_cho/workspace/rlwrld_isaac"
...
EXP_NAME="${PARENT_DIR}_eval_${GPU_INSTANCE}_$(date +%Y%m%d%H%M%S)"
...
# ─── 실험 설정 ───
DATA_PATH="/fsx/rlwrld/youngwoong_cho/datasets/v4/v4_cube_box_5cm_left_100_100"
...
mkdir -p "${EXP_DIR}/logs" "/fsx/rlwrld/youngwoong_cho/logs" "${EVAL_DIR}"
...
# ─── Locate final checkpoint (training must have completed first) ───
LAST_CKPT=$(ls -d ${CKPT_DIR}/checkpoint-* 2>/dev/null | sort -t- -k2 -n | tail -1)
if [ -z "$LAST_CKPT" ]; then
    log "ERROR: No checkpoint found in ${CKPT_DIR} — run training (run.sh) or copy checkpoints first."
    exit 1
fi
log "Using checkpoint: ${LAST_CKPT}"
...
agg = {
    'experiment': '${EXP_NAME}',
    'description': 'Yashu baseline_scratch - no torque, no random-diffusion',
