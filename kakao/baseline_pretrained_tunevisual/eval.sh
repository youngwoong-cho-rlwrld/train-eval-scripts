#!/bin/bash
#SBATCH --job-name=baseline_pretrained_tunevisual_eval
#SBATCH --partition=rlwrld
#SBATCH --nodes=1
#SBATCH --gpus-per-node=4
#SBATCH --time=08:00:00
#SBATCH --output=/rlwrld2/home/youngwoong_cho/logs/baseline_pretrained_tunevisual_eval_%j.out
#SBATCH --error=/rlwrld2/home/youngwoong_cho/logs/baseline_pretrained_tunevisual_eval_%j.err
#SBATCH --comment="baseline_pretrained_tunevisual: eval-only on existing final checkpoint"

###############################################################################
# baseline_pretrained_tunevisual: Cube_Box_left baseline_pretrained_tunevisual without torque & random-diffusion
#
# 학습 → 평가 → 결과 집계까지 한번에 실행
# 모든 결과는 CLAUDE_EXPERIMENTS/baseline_pretrained_tunevisual/ 에 저장
#
# 사용법:
#   cd ~/scripts
#   sbatch baseline_pretrained_tunevisual/run.sh
###############################################################################
...
if [ -n "$SLURM_SUBMIT_DIR" ]; then
    PARENT_DIR="${SLURM_JOB_NAME:-baseline_pretrained_tunevisual}"
